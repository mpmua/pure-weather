...
function fetchCityData() {
    fetch(selectedLocation)
        ...
        .then(data => {
          ...
          <br>`;
            ...
            minMaxCurrentDayTemp.innerHTML =
                minMaxCurrentDayTemp.innerHTML = `<div class="current-max-min-temp-wrapper">
          ...
          <div class="current-min-temp">${Math.round(
            ...
            document.querySelector(".feels-like-stat").innerHTML =
                Math.round(feelsLikeTemp) + ` ${selectedTempUnit}`;
            document.querySelector(".wind-stat").innerHTML =
                Math.round(dataCurrenWindSpeed) + ` ${selectedWindSpeedUnit}`;
            document.querySelector(".humidity-stat").innerHTML =
                Math.round(data.current.humidity) + "%";
            document.querySelector(".pressure-stat").innerHTML =
            ...
            if (data.current.weather[0].main == "Rain") {
                ...
                bodyBackgroundColor =
                    "linear-gradient( 135deg, #c065c7 10%, #1904E5 100%)";
            }
        ...
        <div class="single-hour-icon"><i class="wi ${
            dict[weatherIconHourly]
        }"></i></div>
        ...
        <div class="single-hour-pop">${hourlyPop}</div>`;
            ...
            for (d = 0; d < 8; d++) {
                ...
                let dailyWeatherDescription =
                    data.daily[d].weather[0].description;
                let dailyWeatherDescriptionSplit =
                    dailyWeatherDescription.split(" ");
                let dailyWeatherDescriptionFinalArray = [];

                for (t = 0; t < dailyWeatherDescriptionSplit.length; t++) {
                    let dailyWeatherDescriptionFinal =
                        dailyWeatherDescriptionSplit[t]
                            .charAt(0)
                            .toUpperCase() +
                        dailyWeatherDescriptionSplit[t].slice(1);
                    dailyWeatherDescriptionFinalArray.push(
                        dailyWeatherDescriptionFinal
                    );
                }
...
if (localStorage.getItem("savedLocations") !== null) {
    ...
    for (c = 0; c < locallyStoredCities.length; c++) {
        ...
        document.getElementById(c).addEventListener("click", e => {
            ...
            document.getElementById(clicked).style.background =
                "rgba(0, 0, 0, 0.5)";
